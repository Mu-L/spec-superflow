...
Converts planning artifacts into a single execution handshake: `execution-contract.md`. Load the baseline with `npx --yes --package spec-superflow@0.9.0 ssf runtime asset read templates/execution-contract.md`.

Read before generating: `proposal.md`, `specs/`, `design.md`, `tasks.md`, then load `docs/artifact-contract.md` with `npx --yes --package spec-superflow@0.9.0 ssf runtime asset read docs/artifact-contract.md`.
...
```bash
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_3_result "approved: <summary>"
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_3_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
Run `npx --yes --package spec-superflow@0.9.0 ssf state init <change-dir>` to create `.spec-superflow.yaml` with hashes.

For hotfix, after writing the minimal contract, run `npx --yes --package spec-superflow@0.9.0 ssf state init <change-dir>` or `npx --yes --package spec-superflow@0.9.0 ssf state rebuild <change-dir>` so `contract_hash` is recorded. DP-3 remains mandatory before build.
