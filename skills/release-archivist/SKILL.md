...
- Run `npx --yes --package spec-superflow@0.9.0 ssf audit <change-dir>` — include `decision-point-audit.md` in archive

### DP-6 (Verification Outcome)
```bash
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_6_result "<pass|conditional|fail>: <summary>"
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_6_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
```bash
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> test_result pass
```

### DP-7 (Archive Confirmation)
```bash
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_7_result "confirmed: <archive summary>"
npx --yes --package spec-superflow@0.9.0 ssf state set <change-dir> dp_7_timestamp $(date -u +%Y-%m-%dT%H:%M:%SZ)
```
...
Run `npx --yes --package spec-superflow@0.9.0 ssf state transition <change-dir> closing`. If delta specs exist, route to `spec-merger`.
...
- **Missing files**: If audit can't generate, run `npx --yes --package spec-superflow@0.9.0 ssf audit` manually
